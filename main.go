...
var (
	...
	certDir                       = flag.String("cert-dir", "certificates", `Directory for save cached certificates. Set cert-dir=- to disable saving of certificates.`)
	...
	defaultDomain                 = flag.String("default-domain", "", "Usage when SNI domain isn't available (has zero length). For example client doesn't support SNI. It is used to obtain and a certificate only. It isn't force set header HOST in request.")
	...
	logrotateMaxCount             = flag.Int("logrotate-count", 30, "How many old backups to keep. 0 for keep indefinitely.")
